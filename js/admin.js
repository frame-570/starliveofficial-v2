...
async function loadOrders({ silent = false } = {}) {
  ...
  let query = supabase
    .from("orders")
    .select("*, events(title, status, rerun_special_notes, event_days(event_date)), ticket_packages(num_days), ticket_package_day_options(label)")
    .order("created_at", { ascending: false })
    .limit(100);
  ...
  if (copyMessageBtn) {
    copyMessageBtn.addEventListener("click", async () => {
      const lines = await buildCustomerMessageLines({
        ...
        eventStatus: order.events?.status,
        rerunSpecialNotes: order.events?.rerun_special_notes,
      });
...
manualOrderForm.addEventListener("submit", async (e) => {
  ...
  await showManualResult(body, eventId);
...
async function buildCustomerMessageLines({
  eventTitle,
  eventDateLabel,
  packageNumDays,
  dayOptionLabel,
  ...
  const isRerun = eventStatus === "rerun";
  ...
  const lines = [
    ...
    isRerun
      ? `ลิงก์เข้าชม (ใช้ดูรีรัน): ${watchUrl}`
  ...
  if (isRerun) {
    // งานที่เป็นรีรันแล้ว: ไม่ชวน LINE OpenChat อีก โชว์รายละเอียดพิเศษของงานนั้นแทน (ถ้าตั้งไว้)
    const notes = rerunSpecialNotes?.trim();
    if (notes) {
      lines.push(``);
      lines.push(notes);
    }
...
async function showManualResult(result, eventId) {
  let eventStatus = null;
  let rerunSpecialNotes = null;
  if (eventId) {
    const { data: ev } = await supabase.from("events").select("status, rerun_special_notes").eq("id", eventId).maybeSingle();
    eventStatus = ev?.status;
    rerunSpecialNotes = ev?.rerun_special_notes;
  }
...
const discountCodeCreateInput = document.getElementById("discountCodeCreateInput");
...
const discountMaxUsesInput = document.getElementById("discountMaxUsesInput");
...
discountTypeSelect.addEventListener("change", () => {
  discountValueLabel.textContent = discountTypeSelect.value === "percent" ? "ลดกี่เปอร์เซ็นต์ (%)" : "ลดกี่บาท";
  discountValueInput.placeholder = discountTypeSelect.value === "percent" ? "เช่น 10" : "เช่น 50";
});
...
discountForm.addEventListener("submit", async (e) => {
  ...
  const code = discountCodeCreateInput.value.trim();
  ...
  if (!discountValue || discountValue <= 0) {
    discountFormError.textContent = "กรุณากรอกจำนวนส่วนลดให้ถูกต้อง";
    return;
  }
  if (discountType === "percent" && discountValue > 100) {
    discountFormError.textContent = "ลดเป็นเปอร์เซ็นต์ต้องไม่เกิน 100%";
    return;
  }
  if (!maxUses || maxUses <= 0) {
    discountFormError.textContent = "กรุณากรอกจำนวนสิทธิ์ให้ถูกต้อง";
    return;
  }
  ...
  createDiscountBtn.textContent = "สร้างโค้ด";

  if (error) {
    discountFormError.textContent = error.message.includes("duplicate")
      ? "โค้ดนี้มีอยู่แล้ว กรุณาใช้โค้ดอื่น"
      : "สร้างโค้ดไม่สำเร็จ: " + error.message;
    return;
  }
...
function renderDiscountRow(code) {
  ...
  row.innerHTML = `
    <div style="min-width:0;">
      <div style="font-family:'Prompt',sans-serif; font-weight:700; font-size:16px; letter-spacing:0.05em; margin-bottom:6px; color:var(--amber);">
    ...
    <div style="display:flex; align-items:center; gap:8px; flex-shrink:0; flex-wrap:wrap;">
      <button class="icon-btn ghost" data-action="toggle-active">${code.is_active ? "ปิดใช้งาน" : "เปิดใช้งาน"}</button>
      <button class="icon-btn ghost" data-action="delete-discount" style="border-color:var(--crimson); color:var(--crimson);">ลบ</button>
    </div>
  `;
  ...
  row.querySelector('[data-action="toggle-active"]').addEventListener("click", async () => {
    ...
    if (error) {
      await appAlert("อัปเดตไม่สำเร็จ: " + error.message, { type: "error" });
      return;
    }
    loadDiscountCodes();
  });

  row.querySelector('[data-action="delete-discount"]').addEventListener("click", async () => {
    const session = await ensureAuthSession();
    if (!session) return;
    const ok = await appConfirm(`ลบโค้ด "${code.code}" ใช่หรือไม่?`, { danger: true, confirmText: "ลบโค้ดนี้" });
    if (!ok) return;
...
// ---------- หมวด: รายละเอียดพิเศษ (บันทึกแยกต่อ event ไม่รวมกับปุ่มบันทึกหลัก) ----------
const specialEventSelect = document.getElementById("specialEventSelect");
const specialNotesInput = document.getElementById("specialNotesInput");
const specialNotesError = document.getElementById("specialNotesError");
const specialNotesSaved = document.getElementById("specialNotesSaved");
const saveSpecialNotesBtn = document.getElementById("saveSpecialNotesBtn");

async function loadSpecialNotesEventOptions() {
  specialNotesError.textContent = "";
  specialNotesSaved.textContent = "";
  specialNotesInput.value = "";
  saveSpecialNotesBtn.disabled = true;

  const { data, error } = await supabase.from("events").select("id, title").order("created_at", { ascending: false });

  if (error) {
    specialNotesError.textContent = "โหลดรายการงานไม่สำเร็จ: " + error.message;
    return;
  }

  specialEventSelect.innerHTML =
    `<option value="">-- เลือกงาน --</option>` +
    (data || []).map((ev) => `<option value="${escapeAttr(ev.id)}">${escapeHtml(ev.title)}</option>`).join("");
}

specialEventSelect.addEventListener("change", async () => {
  specialNotesError.textContent = "";
  specialNotesSaved.textContent = "";
  specialNotesInput.value = "";
  ...
  const { data, error } = await supabase.from("events").select("rerun_special_notes").eq("id", eventId).maybeSingle();
  if (error) {
    specialNotesError.textContent = "โหลดข้อมูลไม่สำเร็จ: " + error.message;
    return;
  }

  specialNotesInput.value = data?.rerun_special_notes || "";
  saveSpecialNotesBtn.disabled = false;
});

saveSpecialNotesBtn.addEventListener("click", async () => {
  const eventId = specialEventSelect.value;
  if (!eventId) return;
  ...
  specialNotesSaved.textContent = "";
  saveSpecialNotesBtn.disabled = true;
  saveSpecialNotesBtn.textContent = "กำลังบันทึก...";

  const { error } = await supabase
    .from("events")
    .update({ rerun_special_notes: specialNotesInput.value.trim() || null })
    .eq("id", eventId);

  saveSpecialNotesBtn.disabled = false;
  saveSpecialNotesBtn.textContent = "บันทึกรายละเอียดพิเศษ";

  if (error) {
    specialNotesError.textContent = "บันทึกไม่สำเร็จ: " + error.message;
    return;
  }
